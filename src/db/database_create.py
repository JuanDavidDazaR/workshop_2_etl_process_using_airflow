...
def create_database_load():
    ...
    try:
        ...
        with engine.connect() as connection:
            ...
            result = connection.execute(
                text("SELECT 1 FROM pg_database WHERE datname = :db_name_load;"),
                {"db_name_load": db_name_load},
            ).fetchone()
            if not result:
                connection.execute(text(f'CREATE DATABASE "{db_name_load}";'))
                print(f"Database '{db_name_load}' successfully created.")
            else:
                print(f"Database '{db_name_load}' already exists.")
